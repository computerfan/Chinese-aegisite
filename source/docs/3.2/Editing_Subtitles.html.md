...
1. 打开[[颜色选择器|Colour_Picker]]选择颜色后，在光标位置插入插入一个“主要颜色”特效标签(`\c`) 。
1. 打开[[颜色选择器|Colour_Picker]]选择颜色后，在光标位置插入插入一个“次要颜色”特效标签(`\2c`) 。
1. 打开[[颜色选择器|Colour_Picker]]选择颜色后，在光标位置插入插入一个“边框颜色”特效标签(`\3c`) 。
1. 打开[[颜色选择器|Colour_Picker]]选择颜色后，在光标位置插入插入一个“阴影颜色”特效标签(`\4c`) 。
...
* 打开 _字幕_ 菜单, 在 _选择行_ 区域可以筛选字幕行进行选择。
...
**合并行(视为卡拉OK)**
: 与 _分割行(按卡拉OK)_ 功能相反,也就是在 _合并行(连接)_ 的基础上按照被合并各行的时间加入`\k`标签。
